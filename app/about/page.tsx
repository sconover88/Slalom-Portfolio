      ...
      <section className="relative py-12 md:py-16">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          <div className="flex flex-col gap-4 order-2 md:order-1">
            ...
            <p className="font-heading text-lg md:text-xl font-bold text-accent">
                ...
                <div key={exp.client} className="relative">
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="font-body text-sm text-muted-foreground">{exp.role}</p>
                    </div>
                  </div>
                  ...
                  <ul className="ml-6 flex flex-col gap-3">
                    {exp.bullets.map((bullet, i) => (
                      <li key={i} className="relative pl-5 font-body text-sm text-foreground leading-relaxed">
                        <span className="absolute left-0 top-2 h-1.5 w-1.5 rounded-full bg-border" />
                        {bullet}
                      </li>
                    ))}
                  </ul>
          ...
          <div>
              ...
              {SKILLS.map((skill) => (
                ...
                >
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-accent/10">
                    <skill.icon className="h-4.5 w-4.5 text-accent" strokeWidth={2.5} />
                  </div>
                  <span className="font-body text-sm font-medium text-foreground">{skill.label}</span>
                </div>
              ))}
            </div>

            {/* Tools */}
            <h2 className="font-heading text-2xl font-bold text-foreground mt-10 mb-6">
              Tools
            </h2>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={tool}
                  className="inline-flex items-center rounded-full border border-border bg-card px-3 py-1.5 font-body text-xs font-medium text-foreground"
                >
                  {tool}
                </span>
              ))}
